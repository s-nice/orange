; (function ($, window, document, undefined) {
    ...
    Timeline.prototype = {
        ...
        _event: function () {
            var self = this;
            var $obj=self.target.find('.events-header');
            //.data('clickEvt', 1)
        	self.target.find('.events-header').each(function(){
        		if ($(this).data('clickEvt')){
        			return true;
        		}
        		$(this).on('click', function(){
        			$(this).siblings('.events-body').stop(true,true).slideToggle()
                    	.end().siblings('.events-footer').stop(true,true).toggle();
        		}).data('clickEvt', 1);
        	});
