...
$.dialog = {
    ...
    alert: function (set) {
        ...
        $(".dialog").append('<div class="dialog_body"><div class="dialog_top">' + d.title + '</div><div class="dialog_main">' + d.content + '</div><div class="dialog_bottom"><span><input type="button" value="确定"  /></span></div></div>');
        var callback = d.callback;
        $('.dialog_bottom input').on('click', function () {
            $.dialog.close();
    ...
    confirm: function (set) {
        ...
        $("body").append('<div class="dialog" style="width:' + d.width + 'px; height:' + d.height + 'px; position:fixed; top:' + getTop(d.height) + '%; left:' + getLeft(d.width) + '%; z-index:1000;"></div>');
        $(".dialog").append('<div class="dialog_body"><div class="dialog_top">' + d.title + '</div><div class="dialog_main">' + d.content + '</div><div class="dialog_bottom"><span><input type="button" value="确定"  />&nbsp;<input type="button" value="取消" onclick="$.dialog.close();" /></span></div></div>');
        var callback = d.callback;
        $('.dialog_bottom input:first').on('click', function () {
            $.dialog.close();
            if (typeof callback == 'string') {
                eval(callback);
            } else if (typeof callback == 'function') {
