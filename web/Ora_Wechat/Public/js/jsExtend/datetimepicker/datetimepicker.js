			...
			createDateTimePicker = function( input ) {
				...
				datetimepicker
					.on('changedatetime.xdsoft',function() {
						if (options.showWeak) {
							var input_val = datetimepicker.data('input');
							var val= input_val['context']['value'];
							val = val.split(' ');
							var time = Date.parse(val[0]);
							var times = new Date(time);
							var weak = times.getDay();
							var weakName = options.i18n[options.lang].dayOfWeek[weak];
							var yearName = options.i18n[options.lang].YMD[0];
							var monthName = options.i18n[options.lang].YMD[1];
							var dayName = options.i18n[options.lang].YMD[2];
							var month = times.getMonth();
							month = month+1 < 10?'0'+(month+1):(month+1);
							var day = times.getDate();
							day = day < 10 ? '0'+day:day;
							if (val.length > 1) {
								var new_val = times.getFullYear()+yearName+month+monthName+day+dayName+' '+weakName+' '+val[1];
							}else{
								var new_val = times.getFullYear()+yearName+month+monthName+day+dayName+' '+weakName;
							}	     					
	     					var show_id = input_val['context']['id']+'_show';
							$('#'+show_id).val(new_val);	
						};
