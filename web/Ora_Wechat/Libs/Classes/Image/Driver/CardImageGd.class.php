...
import('ORG.Util.Image.Driver.ImageGd');
import('ORG.Util.Image.Driver.GIF');

class CardImageGd extends ImageGd
{
    /**
     * 图像资源对象
     * @var resource
    ...
    {
        if (isset($settings['imagePath'])) {
            $this->open($settings['imagePath']);
        } else if (isset($settings['width'], $settings['height'])
           && intval($settings['width'])>0 && intval($settings['height'])>0) {
            $width = intval($settings['width']);
            $height = intval($settings['height']);
            $bgColor = isset($settings['bgColor']) ? $settings['bgColor'] : '#FFFFFF';
            $this->createConvas($width, $height, $bgColor);
            $imageType = isset($settings['imageType']) ? $settings['imageType'] : IMAGETYPE_PNG;
            $this->_setImageType($imageType);
        }
    }

    protected function createConvas($width, $height, $bgColor='#FFFFFF')
    {
        //销毁已存在的图像
        empty($this->img) || imagedestroy($this->img);
        $this->img = imagecreatetruecolor($width, $height);
        $colorInfo = GFunc::hex2rgb($bgColor);
        if (! $colorInfo) {
            $colorInfo = array('red' => 0, 'green' => 0, 'blue' => 0);
        }
        $color = imagecolorallocate($this->img, $colorInfo['red'], $colorInfo['green'], $colorInfo['blue']);
        imagefill($this->img, 0, 0, $color);
        $this->info = is_array($this->info) ? $this->info : array();
        $this->info['width'] = $width;
        $this->info['height'] = $height;
    }

    private function _setImageType ($imageType)
    {
        switch ($imageType) {
            case IMAGETYPE_GIF:
                $mime = 'image/gif';
        ...
        $this->info = is_array($this->info) ? $this->info : array();
        $this->info['type'] = image_type_to_extension($imageType, false);
        $this->info['mime'] = $mime;
    }
