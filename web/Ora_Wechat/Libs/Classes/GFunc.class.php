...
/*     	$savePath = C('DATA_CACHE_PATH').'/LoginIntercept/'.$usertype;//保存路径
    	...
    	if($rtn){
    		...
    		$result = \AppTools::WebService('\Model\Appadmin\Oauth\Oauth', 'getUserToken', $params);
    		if($result['status']){
        ...
        for ($i = 0; $i < count($match[0]); $i++) {
            ...
            if(strstr($match[1][$i],'http')){//判断是服务器上的图片地址还是本地的图片地址  如果是本地的添加上绝对路径
                ...
                if ($uploadinfo['status'] == 0) {
                    $uploadPaths[] = $uploadinfo['data']['path'];
                }
            }
        }
...
//        $content  = str_replace(array("\r\n","\n",' '), array("<br/>","<br/>","&nbsp;&nbsp;"),$content );
	 ...
	 * @return array
	 * */
	public  static function  getAdminAccountInfo($id){
		$result = \AppTools::webService('\Model\Appadmin\AdminAccount', 'manageAdminAccount',array('params'=>array('adminid'=>$id)));
		if($result['status']==0){
