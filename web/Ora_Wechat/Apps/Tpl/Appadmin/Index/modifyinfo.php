...
<div class="content_global">
	...
	<div class="modifyinfo_time"><span>{$T->end_login_time}：</span><em>{$admin_info['lastlogintime']}</em></div>
	<div class="modifyinfo_IP"><span>{$T->end_login_IP}：</span><em>{$admin_info['lastloginip']}</em></div>
	...
	<div class="modifyinfo_maile"><span>{$T->mobile}：</span><input type='text' name='mobile' value="{$admin_info['mobile']}" /><em id='tx_mobile'></em></div>
	 -->
	<div class="modifyinfo_button"><span></span><button id='js_submit' class="big_button">{$T->modify_info_submit}</button></div>
</div>
<script>
