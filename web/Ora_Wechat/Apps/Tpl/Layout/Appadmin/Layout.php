...
<html>
  <head>
    ...
    <include file="@Layout/Appadmin/head" />
  ...
  <body>
	<div class="appadmin_all_content" id="tbposition" style="position:relative; ">
			...
			<div class="s_content">
				<div class="s_content_wrapper">
					<div class="appadmin_content">
						<!-- 页面头部内容 -->
						<include file="@Layout/Appadmin/appadmin_header" />
						<!-- 页面 正文部分 -->
						<div class="appadmin_section">
							<include file="@Layout/Appadmin/left_navmune" />
							<div class="appadmin_section_right">
								<include file="@Layout/Appadmin/rightnavigation" />
								{__CONTENT__}
							</div>
						</div>
					</div>
		...
		<include file="@Layout/Appadmin/unlockpop" />
	</div>
		<script src="__PUBLIC__/js/jquery/jquery-ui.min.js"></script>
		<script src="__PUBLIC__/js/jsExtend/layer/layer.min.js"></script>
		<script src="__PUBLIC__/js/Appadmin/global.js?v={:C('APP_VERSION')}"></script>
		<if condition="isset($_SESSION[MODULE_NAME])">
			<link href="__PUBLIC__/css/Appadmin/globalPop.css?v={:C('APP_VERSION')}" rel="stylesheet" type="text/css">
			<script src="__PUBLIC__/js/Appadmin/globalPop.js?v={:C('APP_VERSION')}"></script>
...
<!--            <script src="__PUBLIC__/js/jsExtend/jplayer/jquery.jplayer.min.js"></script>-->
		...
		{// 加载本模块js }
		<if condition="is_file(WEB_ROOT_DIR . 'js/Appadmin/'.strtolower(CONTROLLER_NAME).'.js')">
		  <script src="__PUBLIC__/js/Appadmin/{:strtolower(CONTROLLER_NAME)}.js?v={:C('APP_VERSION')}<php>echo time();</php>" charset="utf-8"></script>
		</if>
		...
		<!-- footer部分start -->
		 <include file="@Layout/Appadmin/foot" />
		<!-- footer部分end -->
  </body>
</html>
