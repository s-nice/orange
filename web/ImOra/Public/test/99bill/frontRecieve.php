<?php
	...
	$fp = fopen("./demo/99bill[1].cert.rsa.20140803.cer", "r");
	$cert = fread($fp, 8192);
	fclose($fp);
	...
	if ($ok == 1) {
		switch($_REQUEST['payResult']){
				case '10':
						...
						$rtnUrl="http://dev.orayun.com/test/99bill/show.php?msg=success&oradt=success&orderId=".$_REQUEST['orderId'];
						break;
				default:
						$rtnOK=0;
						//以下是我们快钱设置的show页面，商户需要自己定义该页面。
						$rtnUrl="http://dev.orayun.com/test/99bill/show.php?msg=false&oradt=false";
						break;
						...
						$rtnOK=0;
						//以下是我们快钱设置的show页面，商户需要自己定义该页面。
						$rtnUrl="http://dev.orayun.com/test/99bill/show.php?msg=error&oradt=error";
