    ...
    $.fn.mdatetimer = function(opts){
		...
		$.extend(picker.prototype,
		{
			init : function(){
				...
				var yearScroll = new IScroll('#'+option.id+' .mt_year', scrollConf);
				...
				var monthScroll = new IScroll('#'+option.id+' .mt_month', scrollConf);
				...
				var dayScroll = new IScroll('#'+option.id+' .mt_day', scrollConf);
				...
				if(option.mode != 1){
					...
					var hourScroll = new IScroll('#'+option.id+' .mt_hour', scrollConf);
					...
					var minuteScroll = new IScroll('#'+option.id+' .mt_minute', scrollConf);
					...
					var secondScroll = new IScroll('#'+option.id+' .mt_second', scrollConf);
				...
				//初始化点击input事件
				option.ii.on('tap', function(){
					if(container.hasClass('show')){
						_this.hidePanel();
					}
					else{
						_this.showPanel(container);
				...
				$('.mt_ok', container).on('tap', function(){
					...
					var minute = mpMinute.find('.selected').data('minute');
					var second = mpSecond.find('.selected').data('second');
									console.log(option.ii);
					option.ii.val(F.formatDate(year, month, day, hour, minute, second));
					_this.hidePanel();
				...
				$('.mt_setnow', container).on('tap', function(){
					...
					option.ii.val(F.formatDate(n.getFullYear(), n.getMonth(), n.getDate(), n.getHours(), n.getMinutes(), n.getSeconds()));
					_this.hidePanel();
				});
			...
			showPanel : function(container){
				...
				//container.prev('.mt_mask').addClass('show');
			},
			hidePanel : function(){
				$('.mt_poppanel, .mt_mask').removeClass('show');
			},
			...
			updateDay : function(mpYear, mpMonth, mpDay){
				...
				this.dayScroll = new IScroll('#'+option.id+' .mt_day', scrollConf);
