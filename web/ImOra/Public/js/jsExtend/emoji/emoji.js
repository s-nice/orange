...
//html标签转换为emoji
function htmlToEmoji($orgObj){
	$obj=$orgObj.clone();
	var $spans=$obj.find('span');
	for(var i=0;i<$spans.length;i++){
		var $span=$($spans[i]);
		for(var key in jEmoji.EMOJI_MAP){
			if ($span.attr('title')==jEmoji.EMOJI_MAP[key][1]){
				$span.after($("<aa>"+key+"</aa>"));
				$span.remove();
			}
		}
	}
	var result=$obj.html().replace(/<aa>/gi,'');
	result=result.replace(/<\/aa>/gi,'')
	return result;
}
jEmoji.htmlToEmoji = htmlToEmoji;
