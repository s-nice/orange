...
$('form[name="headForm1"]').find('.time_duibi').remove();
$('form[name="headForm1"]').find('input[name="duibi"]').remove();
...
// 开始对比 等同点击提交表单按键
$('.js_submit_duibi').on('click',function(){
	$('.js_submit_form').trigger('click');
});
//切换日  月  周
...
//    if($('.time_duibi').is(":visible")){   //判断是否有对比时间
//        var  constarttime  =  $('#js_begintime1').val();
//        var  conendtime  =  $('#js_endtime1').val();
//        if(conendtime != '' || constarttime != ''){
//            $('form[name="headForm"]').append('<input name="duibi" type="hidden" value="duibi" />');
//        }else{ //两个时间如果都未填写也不进行对比
//            $('form[name="headForm"]').find('input[name="duibi"]').remove();
//        }
//    }else{ //对比弹框未显示  不进行对比
//        $('form[name="headForm"]').find('input[name="duibi"]').remove();
//    }
