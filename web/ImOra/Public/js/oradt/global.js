...
$(function(){
	...
	//列表页面 滚动表头浮动效果
/*	if($('.js_list_name_title').length>0){
		var $obj=$('.js_list_name_title');
		var e_top= $obj.offset().top;//表头距离浏览器的默认高度
		var m_top= $obj.parent().offset().top+ $obj.parent().height()-($obj.height()*2);//最大悬浮高度
		$obj.addClass('title-top-bg');
		$(document).scroll(function(){
			if(e_top<$(this).scrollTop() && m_top> $(this).scrollTop()){
				$obj.offset({ top: $(this).scrollTop()});
			}else{
				$obj.offset({ top:e_top});
			}
		});
	}*/
