...
function _processDate($startDate, $endDate, $statType='d'){
	...
	if(empty($startDate) && empty($endDate)){
		...
		switch ($statType){
			...
			default:
				$endDate 	= date('Y-m-d',strtotime('-1 day'));
				$startDate	= date('Y-m-d',strtotime('-29 days'.$endDate));
		}
	} else {
		switch ($statType){
			case 'w':
...
function _firstDateOfWeek($date){
   ...
   /**   加密函数
	 ...
	 * @throws Exception
	 */
	function exeCurl($url,  $params = array(), $method = 'GET', $extra_conf = array())
	{
		//Log::write('-------exec $$$$$$$$$$$---- '.print_r(func_get_args(),1));
		$params = is_array($params)? http_build_query($params): $params;
		//如果是get请求，直接将参数附在url后面
		if(strtoupper($method) == strtoupper('GET'))
		{
	...
	//获取微信token
	function getWxTokenToLocal($flush=0,$source=''){
		$url = C('GET_ORADT_WEIXIN_TOKEN_URL').U(MODULE_NAME.'/Wechat/getWxTokenForExternal',array('source'=>$source));
		$rstToken = exeCurl($url,array('flush'=>$flush,'tb'=>1));
		$rstToken = json_decode($rstToken,true);
		//$session = session(MODULE_NAME);
		//$session['access_token']   = $rstToken['access_token'];
		//$session['expires_in']     = $rstToken['expires_in'];
		//session(MODULE_NAME,$session);
		//Think\Log::write(' 内部通过外网获取token : '.print_r($rstToken,1));
		Think\Log::write('File:'.__FILE__.' LINE:'.__LINE__." 本地项目获取微信token \r\n<pre>".''.var_export(array($url,$rstToken),true));
		return $rstToken;
	}
