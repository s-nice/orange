	...
	<body>
		<section class="file_card">
			<div class="warning_text">
				<p><input type='text' id='name' value='张三'><button id='chooseImg'>上传照片</button></p>
			</div>
			<div class="file_cntent" id='uploadLocalImg'>
                <if condition="$data['avatarpath'] eq ''">
                <div class="file_icon" >
					<span>{$T->str_scannercard_cardfront}</span>
					<img src="__PUBLIC__/images/wei/iconCameraL@3x.png" alt=""  />
				</div>
                <else/>
                    <img src="{$data.avatarpath}" alt=""  style='width:100%'/>
                </if>
			</div>
...
<script>
    ...
    var gUploadImgUrl = "{:U('Demo/Face/saveFace')}"; //上传图片
    ...
    wx.config({
        ...
        $(function(){
            ...
            $('#chooseImg, #uploadLocalImg').click(function(){
                var name=$('#name').val();
            	openCamera(name);
            });
