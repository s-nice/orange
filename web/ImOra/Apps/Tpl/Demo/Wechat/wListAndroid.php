<!DOCTYPE html>
<html lang="en" style=" width:100%;height:100%;display:flex;flex-direction:column;overflow:hidden;background:#111;">
<head>
	<meta charset="UTF-8">
	<!-- uc强制竖屏 -->
...
<body style="width:100%;height:100%;display:flex;flex-direction:column;overflow:hidden;background:#111;">
	<div class="warmp">
		...
		<div class="search_top">
            ...
            <input id="weixinVoiceTime" type="hidden" value=""/>
             ...
             <input id="android" type="hidden" value="1"/>
        ...
        <div class="content_img swiper-container swiper-container-horizontal swiper-container-3d swiper-container-coverflow js_out_div js_page_numb" data-pagenumb="1">
				<ul class="swiper-wrapper js_list" id="js_scroll">
	                <foreach name='list' item='v'>
	                    <li t1="{$v['isfb']}" class="swiper-slide js_li" style="z-index:1;" data-id="{$v.cardid}" frontUrl="{:U('Demo/Wechat/wDetailZp')}?android=1&cardid={$v.cardid}&kwd={:urlencode($keyword)}" backUrl="{:U('Demo/Wechat/detailBack')}?android=1&cardid={$v.cardid}&kwd={:urlencode($keyword)}">
	                        <php>
	                        	if($v['isfb'] == 'front'){
	                        </php>
                            	<a href="{:U('Demo/Wechat/wDetailZp')}?android=1&cardid={$v.cardid}&kwd={:urlencode($keyword)}" id="jsDetailId" dd='a'>
                           	 ...
                           	 <php>}else{</php>
                            	<a href="{:U('Demo/Wechat/detailBack')}?android=1&cardid={$v.cardid}&kwd={:urlencode($keyword)}" id="jsDetailId" dd='b'>
		...
		<button type="button" class="b_sound voiceId weui-btn weui-btn_primary"  style="display: none;">
				<div class="sound_btn">
					<span><img src="__PUBLIC__/images/soud.png" ></span>
...
var gVcardListSearchUrl = "{:U(MODULE_NAME.'/Wechat/wListZp',array('keyword'=>$keyword,'openid'=>$openid,'android'=>$isAndroid),'',true)}";
...
$(function(){
    ...
    //ajax分页
/*     function ajaxPage(rst){
        if(rst.status == 0){
            if(rst.data.numfound>0){
                var list = rst.data.wechats;
                var htmlStr = '';
                var gVcardDetailUrl = gVcardDetailUrl.replace('.html','');
                for(var i in list){//gVcardDetailUrl
                    var obj = list[i];
                    tempUrl = gVcardDetailUrl+'/cardid/'+obj['cardid'];
                    var picture = obj['picture'];
                    var cardid  = obj['cardid'];
                    htmlStr+='<li class="swiper-slide js_li"><a href="'+tempUrl+'"><img class="js_img" src="'+picture+'"></a><span data-id="'+cardid+'" class="remove_card js_btn_remove">删除</span></li>'
                }
                $('#js_list').append(htmlStr);
                totalPage = rst['totalPage'];
                currPage = rst['currPage'];
            }
        }
    } */
...
/*     function  clickObj(){
        if(currPage<totalPage){
            alert(2)
            var currIndex = $('.js_li').index();
            if(rows-currIndex==2){
                alert(3)
                var keyword = $('#search').val();
                var openid = $('#openid').val();
                var data = {page:currPage+1, keyword:keyword, openid:openid};
                $.get(gVcardListUrl,data,function(rst){
                    ajaxPage(rst);
                });
            }
        }
