	...
	<body>
		<div class="cards">
					...
					<div class="nav-list swiper-container swiper-container-horizontal swiper-container-free-mode">
						<ul class="swiper-wrapper">
							<li class="swiper-slide">
								<a class="on js_top_menu" p="1" data-id="0">{$T->str_scannerall_lately}</a>
							</li>
			...
			<section class="card-list js_div_data">
				...
				<else/>
					<foreach name="list" item="vo" key='k'>
						<li class="js_li"><img class="js_img_ori" src="{$vo.picturea}" alt="" /><if condition="($vo['batchid'] eq $batchid) and $batchid"><span>{$T->str_scannerall_new}</span></if></li>
					</foreach>
				</empty>
				</ul>
			</section>
			...
			<footer class="pay_btn pay_btn_bg">
       	 		<button class="btn btn_bottom js_redirect_list" type="button">名片搜索</button>
      		</footer>
		</div>
		...
		<script>
		  ...
		  var gVcardListUrl = "{:U(MODULE_NAME.'/ConnectScanner/listAnySweep','','',true)}"; //任意扫搜索列表
			...
			$(function(){
				//搜索任意扫
		    	$('.js_redirect_list').click(function(){
					window.location.href = gVcardListUrl;
		        });
				//顶部菜单栏切换功能
