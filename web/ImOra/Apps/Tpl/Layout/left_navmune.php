<div class="appadmin_section_left">
  ...
  <h2>{$T->str_news}</h2>
  <ul>
  	<volist name="leftMenu2['menus']" id="menu_val" length="4" offset="0">
		<if condition="linkGetAccess($menu_val['link'])">
			<li class="{: isset($menu_val['active'])?$menu_val['active']:''}"><i><a href="{:U($menu_val['link'],'','',true)}">{$menu_val['title']}</a></i>
				<if  condition="isset($leftType) AND $leftType eq 'news__DEL' ">
					<em>{$leftCounts[$key]}</em>
				</if>
			</li>
		</if>
  	</volist>
  ...
  <h2  class="{:isset($leftMenu2['menus']['collection']['active'])?$leftMenu2['menus']['collection']['active']:''}"><a href="{:U(MODULE_NAME.'/Collection/index')}">{$T->info_content_collection}</a></h2>
  <h2>{$T->str_show_comment}</h2>
  <ul>
  	<volist name="leftMenu2['menus']" id="menu_val2" length="3" key="k" offset="4">
		<if condition="linkGetAccess($menu_val2['link'])">
			<li class="{: isset($menu_val2['active'])?$menu_val2['active']:''}">
				<i><a href="{:U($menu_val2['link'],'','',true)}">{$menu_val2['title']}</a></i>
				<if condition="isset($leftType) AND $leftType eq 'comment'">
					<em>{$leftCounts[$k-1]}</em>
				</if>
			</li>
		</if>
  	</volist>
  </ul>
  <h2 class="{:isset($leftMenu2['menus']['label']['active'])?$leftMenu2['menus']['label']['active']:''}"><a href="{:U(MODULE_NAME.'/News/label')}">{$T->str_label_manage}</a></h2>
-->
