<form class="form_marginauto js_search_form"  method='get'>
    <div class="content_search">
        <div class="search_right_c">
            <div class="js_modelversion select_IOS menu_list input_s_width select_option js_s_div" title="{:$_GET['h_versions']?$_GET['h_versions']:'全部系统平台'}">
                <input type="text" name="channel" value="{:$_GET['h_versions']?$_GET['h_versions']:'全部系统平台'}"  readonly="readonly"  alltext='全部系统平台' autocomplete='off'/>
    ...
    $(function(){
        ...
        function setValues($ul){
            ...
            if (isAllChecked){ //全选
                ...
                //橙脉统计  软硬件版本 选择 一个不为全部  其他都是全部 不可同时复选
                var $other=  $('.js_s_div').find('ul input:first').not($ul.parents('.js_s_div').find('ul input:first'));
                $other.each(function(){
                    if(!$(this).is(':checked')){
                        $(this).click();
                    }
