        ...
        <div class="content_c">
            ...
            <div id="userStatisticsData" class="">
            	...
            	<div class="Data_c_content">
            			<div class="Data_cjs_name cards_content_7">
		                  <span class="span_c_1">{$T->stat_date}</span>
		                  <span class="span_c_2">{$T->stat_send_recommend_friendnum}</span>
		                  <span class="span_c_3">{$T->stat_send_recommend_friend_succnum}</span>
		                  <span class="span_c_4">{$T->stat_send_recommend_friend_succprop}</span>
		                </div>
		                <div class="js_scroll_data ">
		            <if condition="count($tableArr) gt 0">
		                <foreach name="tableArr" item="v" key='time'>
			                <div class="Data_c_list_z cards_content_7">
			                  <span class="span_c_1">{$v['time']}</span>
			                  <span class="span_c_2">{$v['recommendnum']}</span>
			                  <span class="span_c_3">{$v['succnum']}</span>
			                  <span class="span_c_4">{$v['prop']}</span>
			                </div>
		                </foreach>
		            <else/>
		              No Data
