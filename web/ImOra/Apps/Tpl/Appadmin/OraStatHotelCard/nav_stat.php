<form class="form_marginauto js_search_form"  method='get'>
      <div class="content_search">
        <div class="search_right_c">
                  <div <if condition="$type eq 5">style="display:none;"</if> class="js_proversion select_IOS menu_list select_option input_s_width js_s_div">
          ...
          <div <if condition="$type eq 5">style="display:none;"</if> class="js_modelversion select_IOS menu_list input_s_width select_option js_s_div">
            <input type="text" name="channel" value="{:$_GET['h_versions']?$_GET['h_versions']:'全部硬件版本'}"  readonly="readonly"  disabled="disabled" alltext='全部硬件版本' autocomplete='off'/>
            <i><img src="__PUBLIC__/images/appadmin_icon_xiala.png" /></i>
                        <b></b>
            ...
            <ul class="js_scroll_list" style="max-height:300px;overflow:hidden;">
                  <li title="全部" val="all"><label><input class="js_i_h"  type="checkbox" >全部</label></li>
                              ...
                              <li title="{$v}"><label><input type="checkbox"  value="{$v}" autocomplete='off'<if condition="in_array($v,$h_versions_check)">checked="checked"</if>>{$v}</label></li>
                              </foreach>
            </ul>
          </div>
...
<else />
  <script>
    $(function(){
      $('#js_endtime').datetimepicker({format:'Y-m-d',timepicker : false,lang:'ch'});
    });
  </script>
</if>
<script>   
