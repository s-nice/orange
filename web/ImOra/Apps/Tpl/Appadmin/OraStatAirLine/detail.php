		...
		<div class="content_c">
			...
			<div class="form_margintop">
				<div class="content_search">
					<div class="search_right_c">
						...
						<div class="select_label menu_list js_select_type" >
							...
							<if condition="$params['is_stop'] eq 1">
								value="是"
				...
				<div class="select_label select_l_w menu_list js_select_type" >
					...
					<if condition="empty($params['legType'])">
						...
						value="国际"
						<elseif condition="$params['legType'] eq 3"/>
						value="未知"
			...
			<div id="userStatisticsData">
				<div class="Data_bt" style="margin-top:0;">
					<form class="js_download" method="post" target="_blank" action="/Appadmin/OraStatAirLine/detail">
						<input type="hidden" value="1" name ="downloadStat"/>
						<input type="hidden" value="{$params['startTime']}" name ="startTime"/>
						<input type="hidden" value="{$params['endTime']}" name ="endTime"/>
						<input type="hidden" value="{$params['flight_num']}" name ="flight_num"/>
						<input type="hidden" value="{$params['goff_date']}" name ="goff_date"/>
						<input type="hidden" value="{$params['is_stop']}" name ="is_stop"/>
						<input type="hidden" value="{$params['legType']}" name ="legType"/>
				...
				<div class="table_content">
					<div class="table_list table_tit">
						<span class="span_t2"><if condition="$params['type'] eq 'pull'">调用<else/>推送</if>订阅时间</span>
