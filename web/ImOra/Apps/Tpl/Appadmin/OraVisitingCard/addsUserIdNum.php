            ...
            <div id="userStatisticsData">
                <div class="Data_bt">
                    ...
                    <if condition="count($tableArr) gt 0">
            			...
            			array('download'=>1,'endTime'=>$params['endTime'],'s_versions'=>$params['s_versions'],'h_versions'=>$params['h_versions'],'itemKey'=>$itemKey))}"><button class="right_down" id="js_download" style="cursor: pointer;">{$T->str_export}</button></a>
                    </if>
                </div>
                <div class="table_content">
                    <div class="table_list table_tit">
                        <span class="span_t2">日期</span>
                        <span class="span_t2">总用户数</span>
                        <span class="span_t2">1个</span>
                        <span class="span_t2">2个</span>
                        <span class="span_t2">3个</span>
                    </div>
                    <div class="clear" id="js_scroll_dom" style="max-height:438px;">
                        <if condition="count($tableArr) gt 0">
                            <foreach name="tableArr" item="v">
                                <div class="table_list">
                                    <span class="span_t2">{$v.time}</span>
                                    <span class="span_t2">{$v.num}</span>
                                    <span class="span_t2">{$v.num1}</span>
                                    <span class="span_t2">{$v.num2}</span>
                                    <span class="span_t2">{$v.num3}</span>
                                </div>
                            </foreach>
                            <else />
                            No Data
