        ...
        <div class="content_c">
            <div class="form_margintop">
                ...
                <div class="js_stat_date_type">
                    <a  <if condition="$period eq 'd' ">class="on"</if> val="d">日</a>
            ...
            <div id="userStatisticsData">
                <div class="Data_bt">
                    <span class="left_s">{$title}数据表</span>
                ...
                <div class="table_content">
                    <div class="table_list table_tit">
                        <span class="span_t9">{$headers.dt}</span>
                        <span class="span_t9">{$headers.pro_version}</span>
                        <span class="span_t9">{$headers.model}</span>
                        <span class="span_t9">{$headers.num}</span>
                        <span class="span_t9">{$headers.num1}</span>
                        <span class="span_t9">{$headers.num2}</span>
                    </div>
                        ...
                        <if condition="count($tableData) gt 0">
                            <volist name="tableData" id="list">
                                <div class="table_list">
                                    ...
                                    <span class="span_t9" title="{$list.num1}">{$list.num1}</span>
                                    <span class="span_t9" title="{$list.num2}">{$list.num2}</span>
                                </div>
                            </volist>
                            <else/>
                            NO DATA
...
<script>
    ...
    var names={all:"{$headers['num']}",text:"{$headers['num1']}",voice:"{$headers['num2']}"};
    $(function(){
