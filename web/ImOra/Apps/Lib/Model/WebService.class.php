...
import('CacheRedis', LIB_ROOT_PATH.'Classes/Classes');
...
class WebService
{
    ...
    public function request($uri=null, $crudMethod=null, $params=array(), $headers=array())
    {
        ...
        $this->actionDemoLogs($microtime); //redis记录日志
        return $result;
    ...
    /**
     * 记录微信操作日志
     * @param int $microtime 发起请求时的时间戳
     *
     */
    public function actionDemoLogs($calltime){
    	if((strtolower(MODULE_NAME) != 'demo')){ 
    		return false;
    	}else if(!\Classes\CacheRedis::ping()){//  || !\Classes\CacheRedis::pingDb()
    		return false;
    	}
        //$microtime = explode(' ', $microtime);
        //$now = explode(' ', microtime());
        //$calltime = $microtime[1]+$microtime[0];
        $logtime = microtime(true);//$now[1]+$now[0];

        $info = array();
        if (!empty(session(MODULE_NAME)['openid'])) {
            $info['user_id'] = session(MODULE_NAME)['openid'];
        } else if(is_array($this->httpParams)){
            $paramArr = $this->httpParams;
            if (isset($paramArr['wechatid'])) {
                $info['user_id'] = $paramArr['wechatid'];
            }
        } else {
            if ((gettype($this->httpParams) == 'string')) {
                $paramArr = json_decode($this->httpParams,true);
                if ($paramArr) {
                    if (isset($paramArr['wechatid'])) {
                        $info['user_id'] = $paramArr['wechatid'];
                    }
        ...
        $info['api_name'] = str_replace(C('WEB_SERVICE_ROOT_URL_WECHAT'), '', $this->httpUri);
        $info['method'] = $this->httpMethod;
        $info['parameter'] = is_array($this->httpParams)?json_encode($this->httpParams, true):$this->httpParams;
        $info['response'] = $this->httpResponse?$this->httpResponse->getBody():'';//$result;
        $info['calltime'] = round($calltime*1000);
        $info['using_time'] = round($logtime*1000)-round($calltime*1000);
        \Classes\CacheRedis::insertList('wx_api',$info);
    }
