...
foreach ($arr as $v){
	...
	foreach ($nodeArr as $nodeV){
		switch($nodeV['labels'][0]){
			case 'User':
				...
				$nodes[$nodeV['id']] = array('category'=>1,'draggable'=>false,
				'symbol' => 'image://http://192.168.30.191:9999/account/avatar?path='.$nodeV['properties']['userid'],
				'symbolSize'=>[30,30],
				'name'=>$nodeV['properties']['name'],'type'=>'user','color'=>'#f39707');
				break;
			case 'Company':
				$nodes[$nodeV['id']] = array('category'=>2,'draggable'=>false,
				'symbol' => 'image://http://jiyanli.oradt.com/test/echarts/image/ps3.png',
				'symbolSize'=>[50,50],
				'name'=>$nodeV['properties']['name'],'type'=>'company','color'=>'#006538');
				break;
			case 'Card':
				$nodes[$nodeV['id']] = array('category'=>3,'draggable'=>false,
	...
	foreach ($linkArr as $linkV){
		...
		$links[] = array('source'=>$nodes[$linkV['startNode']]['name'], 'target'=> $nodes[$linkV['endNode']]['name'], 'name'=>$linkV['type'],
		);
	}
}
// 临时显示查找数据信息
...
// 	// 临时显示查找数据
// 	if($v['type'] == 'user'){
// 		$links[] = array('source'=>'查找人物', 'target'=> $v['name'], 'name'=>'Friend');
