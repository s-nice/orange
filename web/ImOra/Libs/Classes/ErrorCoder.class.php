...
final class ErrorCoder
{
    ...
    static public function getErrorDescByCode($errorCode)
    {
        ...
        foreach ($constantList as $_constKey => $_constValue) {
            if(is_int($_constValue) && $_constValue == $errorCode) {
                $errorDesc = $translator->_($_constKey);
                break;
            }
        }

        if((null===$errorDesc || $_constKey==$errorDesc) && isset($constantList['ERR_DESC_'.$errorCode])) {
