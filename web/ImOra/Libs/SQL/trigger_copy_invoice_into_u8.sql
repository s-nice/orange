...
-- 給u8中间表添加发票id字段
ALTER TABLE lcwy_fp ADD COLUMN `invoice_id` char(40) NOT NULL DEFAULT '' COMMENT '发票id' AFTER `id
-- alter TABLE lcwy_fp DROP COLUMN invoice_id;
...
-- 将数据同步到 U8
        INSERT INTO u8orderdb.lcwy_fp (invoice_id,ccode, ywtype, kpmoney, fpcode, kpname, kpaddress, khbank, khaccount, cstate) 
        SELECT ii.invoice_id as invoice_id,
               ior.order_id AS cccode, 
